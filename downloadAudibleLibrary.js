async function initAubibleLibraryMiner() {
  ...
  function convert2TsvAndDownload(records, named_file) {
    ...
    const str = (o) =>
      typeof o == "object"
        ? tsvReady(JSON.stringify(o).replace(/\n|\r/g, " "))
        : o;
  ...
  const getCatType = (subcat) =>
    all_cats.filter((r) => r.category === subcat).length &&
  ...
  function parseBookDetails(doc) {
    let publisher_summary = cn(doc, summary)[0]
      ?.innerText?.trim()
    ...
    var categories =
      cn(doc, categories_class)[0] &&
      Array.from(tn(cn(doc, categories_class)[0], "a")).length
        ? Array.from(tn(cn(doc, categories_class)[0], "a")).map((a) =>
            a.innerText.trim()
          )
        : [];

    var date = cn(doc, release_date_class)[0]
      ? dateString(
          cn(doc, release_date_class)[0]
            .innerText.replace(/.+date:/i, "")
            .trim()
        )
      : "";

    let extra_cats = Array.from(cn(doc, "bc-chip-text"))?.map((i) =>
      i.getAttribute("data-text")
    );

    var audible_og_cats = Array.from(
      tn(cn(doc, "categoriesLabel")?.[0], "a")
    ).map((i) => i.innerText);
    ...
    return cleanObject({
      ...
      duration_minutes: cn(doc, runtime_class)[0]
        ? lengthOfBookInMinutes(
            cn(doc, runtime_class)[0]
              .innerText.replace(/length:/i, "")
              .trim()
          )
        : "",
      language: cn(doc, language)?.[0]
        ?.innerText?.replace(/[\s\n\r]*language:[\s\n\r]+/gi, "")
        ?.trim(),
      release_date: date,
      release_timestamp: date ? new Date(date).getTime() : "",
      publisher: cn(doc, publisher_class)[0]
        ? cn(doc, publisher_class)[0]
      ...
      rating: tryFloat(
        /[\d\.]+/.exec(
          cn(cn(doc, "ratingsLabel")?.[0], "bc-pub-offscreen")?.[0]?.innerText
        )?.[0]
      ),
      num_ratings: tryFloat(
        /[\d,]+/
          .exec(
            cn(cn(doc, "ratingsLabel")?.[0], "bc-color-link")?.[0]?.innerText
          )?.[0]
          ?.replace(/\D+/)
      ),
      critic_summary: cn(doc, "productCriticsSummary")?.[0]
        ?.innerText?.trim()
  ...
  function parseLibraryPage(doc) {
    return cn(doc, "adbl-library-content-row").length
      ? Array.from(cn(doc, "adbl-library-content-row")).map((card) => {
          ...
          return {
            url:
              cn(ul, title_class)[0] && cn(ul, title_class)[0].parentElement
                ? cn(ul, title_class)[0].parentElement.href?.replace(/\?.+/, "")
                : "",
            title: cn(ul, title_class)[0]
              ? cn(ul, title_class)[0].innerText.trim()
              : "",
            author:
              author && cn(author, "bc-color-base")[0]
                ? cn(author, "bc-color-base")[0].innerText.trim()
                : "",
            narrator:
              narrator && cn(narrator, "bc-color-base")[0]
                ? cn(narrator, "bc-color-base")[0].innerText.trim()
                : "",
            series:
              series && tn(series, "a")[0]
                ? tn(series, "a")[0].innerText.trim()
                : "",
          };
        })
      : [];
  ...
  async function loopThroughtAudibleLibrary() {
    gi(document, "downloading_percentage_txt").innerText =
      "Retrieving titles...";
    const doc = await fetchDoc(
    ...
    for (let i = 0; i < total_results; i++) {
      ...
      gi(document, "downloading_percentage_bar").style.width = `${
        download_bar_width * (i / total_results)
      }px`;
      gi(document, "downloading_percentage_bar").style.background =
        i % 2 == 0 ? "#07ba5b" : "#3de367";
      gi(
        document,
        "downloading_percentage_txt"
      ).innerText = `Retrieving titles... ${Math.ceil(
        (i / total_results) * 100
      )}% complete`;
    }
    return contain_arr;
  ...
  async function enrichLibraryInformation(order_information) {
    ...
    gi(document, "downloading_percentage_txt").innerText =
      "Retrieving addtional information on titles...";
    ...
    ).style.width = `${download_bar_width}px`;
    ...
    convert2TsvAndDownload(
      merged_with_orders,
      "audible_export_" + new Date().getTime() + ".tsv"
    );
    if (gi(document, "downloading_notifier"))
      gi(document, "downloading_notifier").outerHTML = "";
  }
    ...
    a(cont, [
      ...
      [
        ...
        `position: fixed; top: 100px; left: ${
          (body_width - download_bar_width) / 2
        }px; width: ${download_bar_width}px; z-index: ${new Date().getTime()}; background: #121212; border: 1px solid #3de367; border-radius: 0.2em;`,
      ],
    ]);
  ...
  async function getAllOrders() {
    ...
    gi(
      document,
      "downloading_percentage_txt"
    ).innerText = `Retrieving last year's purchases...`;
    var unqHsh = (a, o) =>
      a.filter((i) => (o.hasOwnProperty(i) ? false : (o[i] = true)));
    var rando = (n) => Math.round(Math.random() * n);
    var delay = (ms) => new Promise((res) => setTimeout(res, ms));
    ...
    async function getOrderPageByDate(df, p) {
      ...
      var order_date_sel = Array.from(
        tn(gi(doc, "ui-it-purchase-history-date-filter"), "option")
      ).map((t) => t.value);
      var pages = Array.from(
        tn(cn(doc, "purchase-history-pagination-wrapper")?.[0], "a")
      )?.map((r) => /&pn=(\d+)/.exec(r.href)?.[1]);
      var titles = Array.from(tn(tn(doc, "tbody")?.[0], "tr"))
        .map((tr) => {
          let purchase_date = cn(
            tr,
          ...
          return {
            ...
            title: /.+(?=[\s\n]+By:)/.exec(
              cn(tr, "ui-it-purchasehistory-item-title")?.[0]?.innerText
            )?.[0],
            author: /(?<=[\s\n]+By: ).+/.exec(
              cn(tr, "ui-it-purchasehistory-item-title")?.[0]?.innerText
            )?.[0],
      ...
      return {
        ...
        pages: pages?.length ? unqHsh(pages, {}) : [],
      };
    }
    ...
    for (let i = 0; i < first_page.pages.length; i++) {
      let next_page = await getOrderPageByDate(
        "last_365_days",
        first_page.pages[i]
      );
    ...
    for (let y = 0; y < years_loop.length; y++) {
      ...
      gi(document, "downloading_percentage_bar").style.width = `${
        download_bar_width * (y / years_loop.length)
      }px`;
      gi(document, "downloading_percentage_bar").style.background =
        y % 2 == 0 ? "#07ba5b" : "#3de367";
      gi(
        document,
        "downloading_percentage_txt"
      ).innerText = `Retrieving ${years_loop[y]} purchases...`;
      for (let i = 0; i < page.pages.length; i++) {
        ...
        gi(
          document,
          "downloading_percentage_txt"
        ).innerText = `Retrieving ${years_loop[i]} purchases... page: ${page.pages[i]}`;
      }
      await delay(rando(333) + 666);
    }
